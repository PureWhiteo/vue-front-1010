      ...
      <div>
        <article class="c-v-pic-wrap" style="height: 357px;">
          <section class="p-h-video-box" id="videoPlay">
            <img height="357px" width="100%" :src="courseWebVo.cover" :alt="courseWebVo.title" class="dis c-v-pic">
          </section>
        </article>
...
export default {
   asyncData({ params, error }) {
    ...
    created() {
    this.comment.courseId = this.courseIdt
    this.initCourseInfo()
    this.initComment()
  },
  mounted(){
    ...
    console.log(this.courseIdt)
  },
  methods:{
      initCourseInfo(){
        courseApi.getCourseInfo(this.courseIdt)
        .then(response => {
            this.courseWebVo = response.data.data.courseWebVo,
            this.chapterVideoList=response.data.data.chapterVideoList,
            // this.courseIdt=params.id
            this.isBuy=response.data.data.isBuy
        })
      },
      initComment(){
      comment.getPageList(this.page, this.limit, this.courseIdt).then(response => {
        ...
        gotoPage(page){
          comment.getPageList(page, this.limit,this.courseIdt).then(response => {
              this.data = response.data.data
          })
      },
      createOrders(){
        ordersApi.createOrders(this.courseIdt)
        .then(response =>{
          //获取返回的订单号
          //生成订单，跳转到显示页面
